...
available_space = 70000000 - root.root.size
to_be_deleted = 30000000 - available_space
folder_sizes = []
for node in postorder_iter(root):
    if not node.is_leaf:
        folder_sizes.append(node.size)


first_above = min([i for i in folder_sizes if to_be_deleted < i])
print(first_above)
