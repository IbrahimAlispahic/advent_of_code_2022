...
for line in data.splitlines():
    ...
    for m in range(int(no_moves)):
        move(direction, head_coord)
        dist_head_tail = calc_distance(head_coord, tail_coord)

        if prev_head_coord == [0, 0]:
            tail_coord = prev_head_coord.copy()
            position_str = "X" + str(tail_coord[0]) + "Y" + str(tail_coord[0])
            # print(position_str)
            positions_set.add(position_str)

        if dist_head_tail > 1:
            tail_coord = prev_head_coord.copy()
            position_str = "X" + str(tail_coord[0]) + "Y" + str(tail_coord[1])
            # print(position_str)
            positions_set.add(position_str)

        prev_head_coord = head_coord.copy()
